...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        print('entering _internal_predict_3D_3Dconv_tiled............')
        ...
        if use_gaussian and num_tiles > 1:
            if self._gaussian_3d is None or not all(
                ...
                if verbose: print('computing Gaussian 3D')
        ...
        if verbose: print("prediction done",class_probabilities.shape)
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        for m in range(mirror_idx):
            if m == 0:
#                dum = self(x).detach().cpu().numpy()[0,0]
#                print(np.mean(dum),np.sqrt(np.mean(dum*dum)-np.mean(dum)*np.mean(dum)),np.min(dum),np.max(dum),dum.shape)

#                print(self.inference_apply_nonlin)
                pred = self.inference_apply_nonlin(self(x))
#                dum = pred.detach().cpu().numpy()[0,0]
#                print(np.mean(dum),np.sqrt(np.mean(dum*dum)-np.mean(dum)*np.mean(dum)),np.min(dum),np.max(dum),dum.shape)
