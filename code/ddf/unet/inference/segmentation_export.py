...
from preprocessing.preprocessing import get_lowres_axis, get_do_separate_z, resample_data_or_seg
...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
    ...
    print(np.mean(segmentation_softmax),np.std(segmentation_softmax))
    ...
    print("output image shape", seg_old_spacing.shape,np.mean(seg_old_spacing),np.std(seg_old_spacing))
...
#        seg_old_spacing = seg_old_spacing_final
    ...
    print('bbox',bbox)
    print(seg_old_spacing.shape,np.mean(seg_old_spacing),np.std(seg_old_spacing))

    if bbox is not None:
        seg_old_size = np.zeros((seg_old_spacing.shape[0],) + tuple(shape_original_before_cropping))
        print(f'{shape_original_before_cropping=}',seg_old_size.shape)
        for c in range(3):
